...
const AdminDash = () => {
  ...
  const fetchCandidates = async () => {
    try {
      const response = await axios.get('https://backendvs-2.onrender.com/admin');
  ...
  const editCandidate = async (id, updatedCandidate) => {
    try {
      await axios.put(`https://backendvs-2.onrender.com/admin/${id}`, updatedCandidate);
      fetchCandidates();
  ...
  const deleteCandidate = async (id) => {
    try {
      ...
      await axios.delete(`https://backendvs-2.onrender.com/admin/${id}`);
      fetchCandidates();
    } catch (error) {
